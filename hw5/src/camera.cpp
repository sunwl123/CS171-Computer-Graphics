#include "camera.h"
#include "cloth.h"
#include <memory>
int flag_index = -1;
Vec3 last_position;
float scale_num = 0;

Camera::Camera(Float fov_y) :
  fov_y(fov_y) {
}

Mat4 Camera::LookAtMat() const {
  return glm::lookAt(transform.position, transform.position + transform.Forward(), Vec3(0, 1, 0));
}

Mat4 Camera::PerspectiveMat() const {
  return glm::perspective(glm::radians(fov_y), aspect, near, far);
}

void Camera::Update() {
  aspect = Float(Input::window_width) / Float(Input::window_height);
  bool mouse_condition = true;
  if(!Input::GetKey(KeyCode::F))
  {
      mouse_condition = false;
  }
  if(!Input::GetMouseButton(GLFW_MOUSE_BUTTON_LEFT))
  {
      mouse_condition = false;
  }
  if(mouse_condition == false)
  {
      if(flag_index > 0)
      {
          cloth->is_fixed_masses[flag_index] = false;
      }
      flag_index = -1;
  }
  if(Input::GetKey(KeyCode::F))
  {
      Input::SetCursorVisibility(true);
      Vec3 pos_x, pos_y, pos_z;
      float angle;
      angle = (float)tanf(fov_y / 2.0f * pi / 180.0f);
      pos_x = (Input::mouse_position.x / ((float) Input::window_width / 2.0f) - 1) * aspect * near * angle * transform.Right();
      pos_y = (Input::mouse_position.y / ((float) Input::window_height / 2.0f) - 1) * near * angle * transform.Up();
      pos_z = near * transform.Forward();
      Vec3 generate_ray, now, camera_pos;
      camera_pos = transform.position;
      now = pos_x + pos_y + pos_z + camera_pos;
      generate_ray = pos_x + pos_y + pos_z;
      float distance = 1e9;
      float cmp_distance = 1e9;
      if(flag_index == -1 && Input::GetMouseButton(GLFW_MOUSE_BUTTON_LEFT))
      {
        UVec2 mass_dim = {40, 30};
        cloth->LocalToWorldPositions();
        for(int ih = 0; ih < mass_dim.y; ++ih)
            for(int iw = 0; iw < mass_dim.x; ++iw)
            {
                size_t index = cloth->Get1DIndex(iw, ih);
                auto cloth_pos = cloth->local_or_world_positions[index];
                auto height = cloth_pos - camera_pos;
                cmp_distance = glm::length(height - glm::dot(height, glm::normalize(generate_ray))
                * glm::normalize(generate_ray));
                if(cmp_distance <= 5e-2)
                {
                    if(distance > glm::dot((cloth_pos - camera_pos), glm::normalize(generate_ray)))
                    {
                        distance = glm::dot((cloth_pos - camera_pos), glm::normalize(generate_ray));
                        flag_index = cloth->Get1DIndex(iw, ih);
                    }
                }
            }
        cloth->WorldToLocalPositions();
        if(flag_index >= 0)
        {
            cloth->is_fixed_masses[flag_index] = true;
            Vec3 normal = Vec3(0.0,0.0,0.0);
            cloth->world_accelerations[flag_index] = normal;
            cloth->world_velocities[flag_index] = normal;
            last_position = now;
            auto numerator = glm::length(cloth->local_or_world_positions[flag_index] - camera_pos);
            auto denominator = glm::length(generate_ray);
            scale_num = numerator / denominator;
        }
      }
      else if(flag_index >= 0)
      {
          Vec3 delta = now - last_position;
          cloth->LocalToWorldPositions();
          cloth->local_or_world_positions[flag_index] += delta * scale_num;
          cloth->WorldToLocalPositions();
          last_position = now;
      }
      return;
  }
  else
  {
      Input::SetCursorVisibility(false);
  }
#ifdef FIRST_PERSON_CAMERA
  Input::SetCursorVisibility(false);
#endif
  int get_w = int(Input::GetKey(KeyCode::W));
  int get_a = int(Input::GetKey(KeyCode::A));
  int get_s = int(Input::GetKey(KeyCode::S));
  int get_d = int(Input::GetKey(KeyCode::D));
  int get_q = int(Input::GetKey(KeyCode::Q));
  int get_e = int(Input::GetKey(KeyCode::E));
  is_speeding = get_w + get_a + get_s + get_d + get_q + get_e > 0;
  if (is_speeding)
    speeding_rate = std::lerp(speeding_rate, one, ACCELERATION * Time::fixed_delta_time);
  else
    speeding_rate = std::lerp(speeding_rate, zero, DECELERATION * Time::fixed_delta_time);
  float moveSpeed =
    Input::GetKey(KeyCode::LeftShift) || Input::GetKey(KeyCode::RightShift) ? MOVE_SPEED_FAST : MOVE_SPEED_SLOW;
  // WASD
  Float movement = speeding_rate * moveSpeed * (Float) Time::delta_time;
  transform.position += transform.Forward() * (Float(get_w) * movement);
  transform.position += transform.Left() * (Float(get_a) * movement);
  transform.position += transform.Back() * (Float(get_s) * movement);
  transform.position += transform.Right() * (Float(get_d) * movement);
  // Q/E
  transform.position += Vec3(0, 1, 0) * (Float(get_e) * movement);
  transform.position += Vec3(0, -1, 0) * (Float(get_q) * movement);
  // rotate
#ifndef FIRST_PERSON_CAMERA
  if (Input::GetMouseButton(1)) {
    Input::SetCursorVisibility(false);
    transform.Rotate(transform.Right(), glm::radians(Input::mouse_position_frame_offset.y * mouse_sensitivity));
    transform.Rotate(Vec3(0, 1, 0), -glm::radians(Input::mouse_position_frame_offset.x * mouse_sensitivity));
  }
  else {
    Input::SetCursorVisibility(true);
  }
#else
  // rotate
  transform.Rotate(transform.Right(), glm::radians(Input::mouse_position_frame_offset.y * mouse_sensitivity));
  transform.Rotate(Vec3(0, 1, 0), -glm::radians(Input::mouse_position_frame_offset.x * mouse_sensitivity));
  // clamp
  Vec3 rotation_euler = glm::eulerAngles(transform.rotation);
  float rotation_euler_degX = glm::degrees(rotation_euler.x);
  float inf, sup;
  if (rotation_euler_degX <= -90) {
    inf = -180;
    sup = -180 + pitch_max;
  }
  else if (rotation_euler_degX <= 0) {
    inf = -pitch_max;
    sup = 0;
  }
  else if (rotation_euler_degX <= 90) {
    inf = 0;
    sup = pitch_max;
  }
  else { // rotation_euler_degX <= 180
    inf = 180 - pitch_max;
    sup = 180;
  }
  rotation_euler.x = glm::radians(glm::clamp(rotation_euler_degX, inf, sup));
  transform.rotation = Quat(rotation_euler);
#endif

}
